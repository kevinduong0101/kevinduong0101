...
ROWS = [
    ("host",),
    ("kv", "Name", "Kevin"),
    ("kv", "Now", "Learning AI Engineering (LLMs)"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Building cool things"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">kevinduong0101@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">kevinduong0101</tspan><tspan fill="{MUTED}">@</tspan>'
